...
export default function TransferPaymentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      // Intentar primero como pack
      try {
        const resPack = await fetch(`/api/packs/${id}`);
        if (resPack.ok) {
      ...
      // Si no es pack, intentar como curso
      try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (isPack) {
          router.push('/compra/pendiente?pack_id=' + id);
        } else {
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link
                href={`/cursos/packs`}
                className="flex-1 bg-[var(--neutral-200)] hover:bg-[var(--neutral-300)] text-[var(--neutral-800)] py-3 px-6 rounded-lg font-medium transition-all text-center inline-flex items-center justify-center"
              >
                Cancelar
              </Link>
            </div>
                  ...
                  <span className="text-2xl font-bold text-[var(--primary)]">{
                    (() => {
                      const finalPrice = course.onSale && course.discountPercentage > 0
                        ? course.price - (course.price * (course.discountPercentage / 100))
                        : course.price;
                      return new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS' }).format(finalPrice);
                    })()
                  }</span>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="bg-white p-8 rounded shadow text-center">
        <h2 className="text-2xl font-bold mb-4">No se encontró el curso o pack</h2>
